                    ...
                    <form action="{{ route('schedule.create') }}" method="POST">
                        ...
                        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                            <div class="px-4 py-5 sm:px-6">
                                <h3 class="text-lg leading-6 font-medium text-gray-900">新規スケジュール家事追加</h3>
                            </div>
                            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <div class="mt-1">
                                    <select id="day_of_week" name="day_of_week" autocomplete="day_of_week" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                        @foreach (App\Models\ScheduleDayofweek::SCHEDULE_DAYOFWEEK_OBJECT as $key => $value)
                                            <option value="{{ $key }}" @if($key === (int)old('day_of_week')) selected @endif>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                    @error('day_of_week')
